...
import { nextSlide,previousSlide,dotSlide } from '../../features/slices/sliderSlice'
...
const Slider = () => {
  ...
  return (
    ...
    {sliderData.map((item)=>{
      ...
      key={item.id}
       className={
        parseInt(item.id) === slideIndex?"opacity-100 duration-700 ease-in-out scale-100" : "opacity-0 duration-700 ease-in-out scale-95"
        }>
        <div>
      ...
      <div className="flex absolute bottom-12  left-[45%]">
        ...
        onClick={()=>dispatch(previousSlide(slideIndex-1))}>
                    <svg
